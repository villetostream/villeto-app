...
import { useObligations } from "@/queries/accounting";
...
export default function BillPayPage() {
  ...
  const obligations = useObligations(legalEntityId);
  ...
  const createRequest = async () => {
    if (!legalEntityId || !obligationId || !requestAmount || !fundingId || !entity) return;
    await run("bill-pay/payment-requests", { legalEntityId, fundingAccountId: fundingId, currency: entity.baseCurrency, amount: requestAmount, paymentMethod: "external_bank", idempotencyKey: `request:${crypto.randomUUID()}`, allocations: [{ financialObligationId: obligationId, amount: requestAmount }] }, "Payment request created as draft");
