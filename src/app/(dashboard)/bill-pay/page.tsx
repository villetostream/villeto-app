...
export default function BillPayPage() {
  ...
  return <div className="min-h-screen bg-dashboard-bg p-6 space-y-6">
    ...
    <Tabs defaultValue="obligations"><TabsList className="flex h-auto flex-wrap"><TabsTrigger value="obligations">Obligations</TabsTrigger><TabsTrigger value="requests">Payment requests</TabsTrigger><TabsTrigger value="approvals">Approvals</TabsTrigger><TabsTrigger value="recording">External recording</TabsTrigger><TabsTrigger value="reconciliation">Reconciliation</TabsTrigger><TabsTrigger value="configuration">Configuration</TabsTrigger></TabsList>
      ...
      <TabsContent value="configuration"><div className="grid gap-4 md:grid-cols-2"><List title="Funding accounts" empty="Add a masked funding reference through Bill Pay configuration.">{(data.funding.data || []).map((item) => <Row key={item.fundingAccountId} title={item.name} detail={`${item.maskedIdentifier} · ${item.currency}`} />)}</List><List title="Vendor beneficiaries" empty="Add masked beneficiary references; plaintext vendor bank fields are not used.">{(data.beneficiaries.data || []).map((item) => <Row key={item.vendorBeneficiaryId} title={item.name} detail={`${item.maskedIdentifier} · ${item.currency}`} />)}</List></div></TabsContent>
    </Tabs>
  </div>;
}
