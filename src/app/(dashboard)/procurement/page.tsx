...
function RecordRow({ href, title, meta, amount }: { href: string; title: string; meta: string; amount: string }) { return <Link href={href} className="group flex items-center justify-between gap-4 px-5 py-4 transition hover:bg-[#f8fbfa]"><div className="min-w-0"><p className="truncate text-[12px] font-semibold text-[#17211d]">{title}</p><p className="mt-1 truncate text-[10px] capitalize text-[#89918d]">{meta}</p></div><div className="flex items-center gap-3"><span className="text-[11px] font-semibold text-[#17211d]">{amount}</span><ArrowRight className="size-3.5 text-[#a6adaa] transition group-hover:translate-x-0.5 group-hover:text-[#087f70]" /></div></Link>; }
...
function Empty({ label }: { label: string }) { return <div className="px-5 py-12 text-center text-[11px] text-[#89918d]">{label}</div>; }
