...
export default function ProcurementInvoicesPage() {
  ...
  return <div className="space-y-5 pb-8">
    ...
    <ProcurementSection title="Invoice review queue" description="Invoices received through the vendor portal">
      ...
      {isLoading ? <div className="flex items-center justify-center gap-2 py-20 text-[12px] text-[#75807b]"><Loader2 className="size-4 animate-spin" /> Loading invoices</div> : isError ? <div className="py-16 text-center"><p className="text-[12px] text-[#b93643]">Unable to load invoices.</p><button onClick={() => refetch()} className="mt-3 text-[11px] font-semibold text-[#087f70]">Try again</button></div> : filtered.length ? <div className="divide-y divide-black/[0.055]">{filtered.map((invoice) => <div key={invoice.vendorInvoiceId} className="grid gap-3 px-5 py-4 transition hover:bg-[#f8fbfa] lg:grid-cols-[1fr_1fr_auto_auto_auto] lg:items-center"><div><p className="text-[12px] font-semibold text-[#17211d]">{invoice.invoiceNumber}</p><p className="mt-1 text-[10px] text-[#89918d]">{invoice.vendor?.displayName || invoice.vendor?.legalName || "Vendor"} · {invoice.poNumber || "Non-PO invoice"}</p></div><div><p className="text-[10px] text-[#89918d]">{invoice.legalEntity.legalName}</p><p className="mt-1 text-[11px] font-semibold text-[#17211d]">{money(invoice.totalAmount, invoice.currency)}</p></div><span className={`w-fit rounded-full px-2.5 py-1 text-[9px] font-semibold capitalize ${badge[invoice.status] || "bg-[#f1f4f3] text-[#68726d]"}`}>{invoice.status.replaceAll("_", " ")}</span><span className="text-[9px] font-semibold capitalize text-[#89918d]">Accounting: {invoice.accountingSyncStatus.replaceAll("_", " ")}</span><div className="flex items-center justify-end gap-2">{invoice.status === "submitted" && can("procurement.vendor_invoice", "review") && <button onClick={() => run(invoice.vendorInvoiceId, "under-review")} className="rounded-[8px] border border-black/[0.08] px-3 py-2 text-[10px] font-semibold">Review</button>}{invoice.status === "under_review" && can("procurement.vendor_invoice", "approve") && <button onClick={() => run(invoice.vendorInvoiceId, "approve")} className="inline-flex items-center gap-1 rounded-[8px] bg-[#087f70] px-3 py-2 text-[10px] font-semibold text-white"><CheckCircle2 className="size-3" /> Approve</button>}<ArrowRight className="size-3.5 text-[#a6adaa]" /></div></div>)}</div> : <div className="py-16 text-center text-[11px] text-[#89918d]">No invoices match this view.</div>}
    </ProcurementSection>
  </div>;
}
