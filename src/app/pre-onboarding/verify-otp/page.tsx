...
export default function VerifyOtp() {
    ...
    const handleProceed = async () => {
        ...
        try {
            ...
            if (isExistingUser && onboardingData) {
                ...
                const company = onboardingData.company as any;

                // ── Core IDs & pre-onboarding contact info ──────────────────────────
                ...
                // ── Business page data (company details) ────────────────────────────
                const isAutoGenerated = company.companyName?.includes("@");
                onboarding.updateBusinessSnapshot({
                    businessName: isAutoGenerated ? "" : (company.companyName ?? ""),
                ...
                // ── Leadership page data (businessOwners) ───────────────────────────
                if (onboardingData.businessOwners && Array.isArray(onboardingData.businessOwners)) {
                    let selfOwnerSet = false;
                    const profiles = [];
                    for (const owner of onboardingData.businessOwners) {
                        const isSelf = owner.email === company.contactEmail;
                        const percentage = typeof owner.ownershipPercentage === "string"
                            ? parseFloat(owner.ownershipPercentage)
                            : (owner.ownershipPercentage || 0);
                        if (isSelf) {
                            onboarding.setSelfOwner({
                                firstName: owner.firstName,
                                lastName: owner.lastName,
                                email: owner.email,
                                ownershipPercentage: isNaN(percentage) ? 0 : percentage,
                            });
                            selfOwnerSet = true;
                        } else {
                            profiles.push({
                                id: owner.userId || `person-${Math.random()}`,
                                firstName: owner.firstName,
                                lastName: owner.lastName,
                                email: owner.email,
                                phone: owner.phone ?? "",
                                ...
                                avatar: `${owner.firstName?.[0] ?? ""}${owner.lastName?.[0] ?? ""}`,
                            });
                        }
                    }
                    onboarding.updateUserProfiles(profiles);
                    if (!selfOwnerSet) {
                        onboarding.setSelfOwner(null);
                    }
                }

                // ── Products page data (productModules) ─────────────────────────────
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col lg:flex-row bg-[#0b100e]">
            <OnboardingSidebar />
            ...
            <div className="flex flex-1 flex-col overflow-y-auto bg-white lg:rounded-l-none">
                ...
                <div className="flex flex-1 flex-col items-start justify-center px-5 py-12 sm:px-12 lg:px-16 xl:px-20">
                    <div className="w-full max-w-[440px]">
                        ...
                        <div className="mt-9">
                            ...
                            <div className="flex items-center gap-1.5 sm:gap-2 w-full max-w-[360px]">
                                {otp.map((digit, index) => (
                                    <React.Fragment key={index}>
                                        ...
                                        <input
                                            ...
                                            className={[
                                                "h-[50px] sm:h-[58px] w-full rounded-[10px] border text-center text-[18px] sm:text-[22px] font-semibold outline-none transition-all duration-150 min-w-0 flex-1",
