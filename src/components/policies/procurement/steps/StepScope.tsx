  ...
  return (
    <div>
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute z-30 left-0 right-0 top-12 rounded-xl border border-border bg-white shadow-lg overflow-hidden">
  ...
  return (
    <div>
      ...
      <div className="max-w-3xl space-y-6">
        ...
        {/* Specific pickers */}
        {scopeType === "specific" && (
          <div className="rounded-2xl border border-border bg-white p-5 space-y-6">
        ...
        <div className="rounded-2xl border border-border bg-white p-5">
          ...
          {hasExceptions ? (
            <div className="flex flex-wrap gap-2 mt-4">
              {(["department", "role", "location"] as ExceptionCategory[]).flatMap((cat) =>
                exceptions[cat].map((item) => (
                  <span
                    key={`${cat}-${item}`}
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-border bg-muted/40 text-xs font-medium text-foreground"
                  >
                    <span className="text-[10px] uppercase text-muted-foreground mr-0.5">{cat}:</span>
                    {item}
            ...
            <div className="rounded-xl border border-dashed border-border bg-muted/20 py-5 text-center mt-4">
              <p className="text-xs text-muted-foreground">No exceptions added</p>
            </div>
          )}
        </div>
      </div>
