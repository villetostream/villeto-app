  ...
  return <section className="space-y-5">
    ...
    <div className="flex flex-col gap-3 rounded-[14px] border border-[#c8eee6] bg-[#f6fcfa] p-5 sm:flex-row sm:items-start sm:justify-between">
      <div className="flex gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[#0b100e]">Where can this vendor be used?</h2>
          <p className="mt-1 max-w-2xl text-sm leading-relaxed text-[#5e6863]">Set up each legal entity in three steps: approve the relationship, assign a vendor site, then enable purchasing.</p>
        </div>
      </div>
...
function CreateRelationshipForm({ entities, submitting, onClose, onCreate }: { entities: LegalEntityOption[]; submitting: boolean; onClose: () => void; onCreate: (legalEntityId: string) => Promise<void> }) {
  ...
  return <><DialogHeader><DialogTitle>Add legal-entity relationship</DialogTitle><DialogDescription>This creates a pending relationship. It is not usable for purchasing until an authorized admin approves it and assigns a site.</DialogDescription></DialogHeader>{entities.length === 0 ? <div className="rounded-[8px] border border-dashed border-black/[0.12] p-4 text-sm text-[#68726d]">Every active legal entity already has a relationship with this vendor.</div> : <div className="space-y-1.5"><label className="block text-sm font-semibold text-[#39423e]">Legal entity</label><Select value={legalEntityId} onValueChange={setLegalEntityId}><SelectTrigger className="w-full h-10"><SelectValue placeholder="Select a legal entity" /></SelectTrigger><SelectContent>{entities.map((entity) => <SelectItem key={entity.legalEntityId} value={entity.legalEntityId}>{entity.legalName} ({entity.code})</SelectItem>)}</SelectContent></Select></div>}<DialogFooter><button type="button" onClick={onClose} className="h-10 rounded-[8px] border border-black/[0.08] px-4 text-sm font-semibold text-[#68726d] hover:bg-[#f9faf9]">Cancel</button><button type="button" disabled={submitting || !legalEntityId} onClick={() => void onCreate(legalEntityId)} className="h-10 rounded-[8px] bg-[#087f70] px-4 text-sm font-semibold text-white hover:bg-[#076b5e] disabled:opacity-50">{submitting ? "Creating..." : "Create pending relationship"}</button></DialogFooter></>;
}
...
function SiteAssignmentForm({ relationship, sites, submitting, onClose, onCreate }: { relationship: EntityRelationship; sites: VendorSite[]; submitting: boolean; onClose: () => void; onCreate: (siteId: string, purpose: AssignmentPurpose, effectiveFrom?: string, effectiveTo?: string) => Promise<void> | void }) {
  ...
  return <><DialogHeader><DialogTitle>Assign a vendor site</DialogTitle><DialogDescription>Authorize one active company-wide site for {entityName} and a specific operational purpose.</DialogDescription></DialogHeader><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className="space-y-1.5"><label className="block text-sm font-semibold text-[#39423e]">Site</label><Select value={siteId} onValueChange={setSiteId}><SelectTrigger className="w-full h-10"><SelectValue placeholder="Select a site" /></SelectTrigger><SelectContent>{sites.map((site) => <SelectItem key={site.vendorSiteId} value={site.vendorSiteId}>{site.name} ({site.code})</SelectItem>)}</SelectContent></Select></div><div className="space-y-1.5"><label className="block text-sm font-semibold text-[#39423e]">Purpose</label><Select value={purpose} onValueChange={(value) => setPurpose(value as AssignmentPurpose)}><SelectTrigger className="w-full h-10"><SelectValue placeholder="Select purpose" /></SelectTrigger><SelectContent>{(["purchasing", "invoicing", "remit_to"] as AssignmentPurpose[]).map((item) => <SelectItem key={item} value={item}>{purposeLabel[item]}</SelectItem>)}</SelectContent></Select></div><Field label="Effective from" value={effectiveFrom} onChange={setEffectiveFrom} placeholder="Leave blank for now" /><Field label="Effective to" value={effectiveTo} onChange={setEffectiveTo} placeholder="Optional" /></div><p className="text-xs leading-relaxed text-[#84908a]">Use ISO dates such as 2026-09-22. A site can be assigned to multiple entities, but each entity and purpose is configured independently.</p>{alreadyAssigned && <p className="rounded-[7px] bg-[#fff9e6] p-2.5 text-xs text-[#80621b]">This site is already active for the selected purpose. Choose another site or purpose.</p>}{invalidDates && <p className="rounded-[7px] bg-[#fdf2f2] p-2.5 text-xs text-[#a3454a]">The end date must be after the start date.</p>}<DialogFooter><button type="button" onClick={onClose} className="h-10 rounded-[8px] border border-black/[0.08] px-4 text-sm font-semibold text-[#68726d] hover:bg-[#f9faf9]">Cancel</button><button type="button" disabled={submitting || !siteId || alreadyAssigned || invalidDates} onClick={() => void onCreate(siteId, purpose, effectiveFrom || undefined, effectiveTo || undefined)} className="h-10 rounded-[8px] bg-[#087f70] px-4 text-sm font-semibold text-white hover:bg-[#076b5e] disabled:opacity-50">{submitting ? "Assigning..." : "Assign site"}</button></DialogFooter></>;
}
