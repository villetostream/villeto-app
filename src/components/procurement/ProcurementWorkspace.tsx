...
export function ProcurementPageHeader({
  ...
  return (
    <section className="flex flex-col gap-4 border-b border-black/[0.07] pb-5 pt-1 sm:flex-row sm:items-end sm:justify-between">
      <div className="max-w-2xl">
        <h1 className="text-[25px] font-semibold tracking-[-0.035em] text-[#10231d] md:text-[28px]">
          {title}
        </h1>
        <p className="mt-2 text-[14px] leading-5 text-[#718079]">{description}</p>
      </div>
      {action && (
        action.href ? (
          <Link
  ...
  return (
    <section className={`overflow-hidden rounded-[15px] border border-black/[0.07] bg-white shadow-[0_12px_35px_-30px_rgba(14,28,23,0.7)] ${className || ""}`}>
      <div className="flex items-center justify-between gap-4 border-b border-black/[0.06] px-5 py-4 shrink-0">
        <div><h2 className="text-[15px] font-semibold text-[#111815]">{title}</h2>{description && <p className="mt-0.5 text-[12px] text-[#84908a]">{description}</p>}</div>
        {action && (
          action.href ? 
